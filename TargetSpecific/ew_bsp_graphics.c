...
#include "ew_bsp_os.h"
  ...
  static XSemaphoreHandle  AcceleratorSemaphore;
...
int EwBspGraphicsInit( void )
{
    ...
    AcceleratorSemaphore = EwBspOsSemaphoreCreate( 1, 0 );
...
void EwBspGraphicsDone( void )
{
  ...
  PXP_Deinit( PXP );
    ...
    /* destroy the accelerator semaphore */
    EwBspOsSemaphoreDestroy( AcceleratorSemaphore );
...
void EwBspGraphicsWaitForCompletion()
{
  ...
  CPU_LOAD_SET_IDLE();
    ...
    EwBspOsSemaphoreWait( AcceleratorSemaphore, 1000 );
...
void PXP_IRQHandler( void )
{
  if (PXP_GetStatusFlags( PXP ) & kPXP_CompleteFlag )
  {
    PXP_ClearStatusFlags( PXP, kPXP_CompleteFlag );
      ...
      EwBspOsSemaphoreRelease( AcceleratorSemaphore );
