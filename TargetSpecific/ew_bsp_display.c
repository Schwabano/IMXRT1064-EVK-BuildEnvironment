...
static void SyncOnLine( int aLine )
{
  ...
  /* line interrupt is configured to begin of vsync area - we have to add vertical
     sync area and vertical back porch to start on the desired visible line */
  aLine += LCD_VSW + LCD_VBP;

  /* calculate time tick that corresponds to requested line */
  ticksTillRequestedLine = aLine * ( VSyncDuration / ( LCD_VSW + LCD_VBP + EW_FRAME_BUFFER_HEIGHT + LCD_VFP ));
...
int EwBspDisplayInit( int aGuiWidth, int aGuiHeight, XDisplayInfo* aDisplayInfo )
{
  ...
  {
    .panelWidth = EW_FRAME_BUFFER_WIDTH,
    .panelHeight = EW_FRAME_BUFFER_HEIGHT,
    .hsw = LCD_HSW,
    .hfp = LCD_HFP,
    .hbp = LCD_HBP,
    .vsw = LCD_VSW,
    ...
    .bufferAddr = ( uint32_t ) EW_FRAME_BUFFER_ADDR,
...
void EwBspDisplayWaitForCompletion( void )
{
    ...
    CPU_LOAD_SET_IDLE();
      ...
      {
        if ( EwBspOsSemaphoreWait( LcdUpdateSemaphore, 1000 ))
          EwBspOsSemaphoreRelease( LcdUpdateSemaphore );
      }
...
void EwBspDisplayCommitBuffer( void* aAddress, int aX, int aY, int aWidth, int aHeight )
{
    ...
    #if EW_USE_OPERATING_SYSTEM == 1
      EwBspOsSemaphoreWait( LcdUpdateSemaphore, 0 );
    #endif
...
void EwBspDisplaySetClut( unsigned long* aClut )
{
  ...
  /* preparing the CLUT in 16bit format in order to fit to LCDIF configuration and
     the connected display */
  for ( i = 0; i < 256; i++ )
    clut[ i ] = ((( aClut[ i ] & 0x00F80000 ) >> ( 3 + 16 )) << 11 ) |
                ((( aClut[ i ] & 0x0000FC00 ) >> ( 2 +  8 )) <<  5 ) |
